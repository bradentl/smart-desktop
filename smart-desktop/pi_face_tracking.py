...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    if trackingFace==0:
        ...
        # user_faces = face_cascade.detectMultiScale(cv2.cvtColor(img, cv2.COLOR_BGR2GRAY), 1.1, 1)
    ...
    if trackingFace == 1:
        ...
        if trackingRes > 8:
            ...
            if(not past_y):
                ...
                # # Move servos
                # bound = lambda t : min(max(t, 0), 240)
                # ser_pos = bound(ser_pos + (10 * dir))
                # servo.move(ser_pos)

                # ensures servo values stays within strict range        
                bound = lambda t : min(max(t,0),240)
                ...
                value = translate(y_center, 0, height, 0, 240)
                ser_pos = bound(value)
                servo.move(ser_pos)
